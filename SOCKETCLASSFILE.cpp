...
int SOCKETCLASS::m_send(const char* senddata)
{
	...
	return send(this->m_socket, senddata, strlen(senddata) + sizeof(char), NULL);//sizeof(senddata), NULL);
}
...
int SOCKETCLASS::m_sendto(const char* senddata, const char* IP, u_short port)
{
	...
	{
		return sendto(this->m_socket, senddata, strlen(senddata)+sizeof(char), NULL, (SOCKADDR*)&this->targetaddr, sizeof(SOCKADDR));//sizeof(senddata), NULL, (SOCKADDR*)&this->targetaddr, sizeof(SOCKADDR));
	}
	else
	{
